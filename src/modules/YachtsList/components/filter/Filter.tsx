...
export default async function Filter({searchParams}: {searchParams: {[key: string]: string}}) {
    ...
    const countries = countriesWhileData.map(country => ({ name: country.name, value: country.id.toString() }));
    const years = getYears().map((year) => ({ name: year.toString(), value: year.toString() }));
    const companies = (await getCompanies()).map(company => ({ name: company.name, value: company.id.toString() }));
    const selectedData = countriesWhileData.find(item => item.id.toString() === searchParams["countries[0][id]"]) || null;
    const ratesOptions = rates.map((rating) => ({ name: rating.name, value: rating.id.toString() }));
    ...
    return (
        <>
            <div className="bg-[#00a0e3] flex flex-col p-5 text-xs">
                ...
                <FilterDate defaultValue={filterDefaults.date} filterName="date" />
                <div className="flex flex-col">
                    <FilterText filterName="days" />
                </div>
            </div>
            <div className="bg-white flex flex-col p-5 py-2 text-xs">
                <FilterSelect filterName="kindId" selectOptions={kinds} description={t("BOAT_TYPE")} placeholder={t('ANY_PLACEHOLDER')} />

                <FilterRangeSlider min={0} max={12} filterName={['personsMin', 'personsMax']} text={t("PERSONS")} />
                <FilterRangeSlider min={0} max={12} filterName={['berthsMin', 'berthsMax']} text={t("BERTHS")} />
                <FilterRangeSlider min={0} max={6} filterName={['cabinsMin', 'cabinsMax']} text={t("CABINS")} />
                <FilterRangeSlider min={0} max={6} filterName={['bathroomsMin', 'bathroomsMax']} text={t("TOLIETS")} />
                ...
                <FilterSelect
                    filterName="ratingMin"
                    selectOptions={ratesOptions}
                    description={t("RATE_NOT_LESS_THAN")}
                    placeholder={t('ANY_PLACEHOLDER')} />

                <FilterSelect
                    filterName="yearMin"
                    selectOptions={years}
                    description={t("NOT_OLDER_THAN")}
                    placeholder={t('ANY_PLACEHOLDER')}/>

                <FilterMinMaxRange
                    label={t('PRICE_WITH_CURRENCY', {currency: '€'})}
                    filterName={['priceMin', 'priceMax']}
                    placeholder={[t("FROM_PLACEHOLDER"), t("TO_PLACEHOLDER")]} />

                <FilterMinMaxRange
                    label={t('LENGTH_WITH_UNIT', {unit: 'M'})}
                    filterName={['lengthMin', 'lengthMax']}
                    placeholder={[t("FROM_PLACEHOLDER"), t("TO_PLACEHOLDER")]} />
                ...
                <div>
                    ...
                    <FilterAutocomplete autocompleteOptions={companies} filterName="companyId" placeholder={t("ANY_PLACEHOLDER")}/>
                </div>
            </div>
        </>
    );
